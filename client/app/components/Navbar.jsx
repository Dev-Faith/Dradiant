...
export const Dropdown = React.memo(function Dropdown({
  ...
  return (
    <m.div
      ...
      animate={{
        height: isDroppeddown ? "auto" : 0,
        opacity: isDroppeddown ? 1 : 0,
      }}
    ...
    >
      <Link href="/pages/profile" onClick={() => setIsDroppeddown(false)}>
        <m.p className="text-[16px] hover:text-[#7B7768] hover:underline hover:text-[#2A4E3A]">
          Profile
        </m.p>
      </Link>
      ...
      {user.role == "admin" && (
        <Link href="/pages/admin" onClick={() => setIsDroppeddown(false)}>
          <m.p className="text-[16px] hover:text-[#7B7768] hover:underline hover:text-[#2A4E3A]">
            Dashboard
          </m.p>
        </Link>
  ...
  const notificationButton = (url) => {
    switch (url) {
      case "/pages/wishlist":
        return (
          <div
            className={` bg-[#7B7768] rounded-full size-[22px] border-[1px] text-[#fff] items-center justify-center ${
              wishListItems.length === 0 ? "hidden" : "flex"
            } cursor-pointer absolute top-[-15px] right-[-15px] text-[16px]/[0] pt-[4px] align-middle`}
          >
  ...
  const LoggedInFeatures = () => {
    return (
      <div className="flex relative min-w-[210px] justify-end items-center gap-[14px] border-l-[1px] border-dashed border-[#CCC6B5] px-[10px] ml-[30px]">
        <p className="greetings w-[100px] truncate">
          Hi,{" "}
          {`${
            user?.user?.firstName !== undefined ? user?.user?.firstName : ""
          } ${
        ...
        <Image
          src={user?.user?.profileImage || defaultProfile}
          alt="profile image"
          width="31"
          height="31"
          className="size-[31px] rounded-full object-cover border-[1px] border-[#1D1C13] cursor-pointer"
        />
        <m.div
        ...
        <AnimatePresence>
          {isDroppeddown && (
            <Dropdown
              ...
              setIsDroppeddown={setIsDroppeddown}
                  ...
                  <Link key={link.name} href={link.url} className="relative">
                    ...
                    <m.p
                      ...
                      className={`cursor-pointer text-center w-full ${
                        link.url !== "/pages/signup"
                          ? "hover:underline hover:text-[#2A4E3A]"
                          : null
                      } ${
                        isActive(link.url) ? " underline text-[#2A4E3A]" : ""
                      } xl:mx-[16px] ${
                        link.url == "/pages/signup"
                          ? "bg-[#6A5F11] text-[#fff] rounded-[10px] p-[8px]"
                          : ""
                      } ${
                        (isAuthenticated && link.url == "/pages/signin") |
                          (isAuthenticated && link.url == "/pages/signup") &&
                        "hidden"
                      }`}
                    >
                      {link.name}
