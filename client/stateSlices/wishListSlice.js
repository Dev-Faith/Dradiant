...
export const addTowishList = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }, { rejectWithValue, dispatch }) => {
    try {
      const response = await axios.post("/api/wishlist/post", {
        userId,
        productId,
        quantity,
      });
...
export const emptyWishlist = createAsyncThunk(
  "wishListSlice/emptyWishlist",
  async (userId, { rejectWithValue, dispatch }) => {
    try {
      const response = await axios.delete("/api/wishlist/delete", {
        data: { userId, empty: true },
      });
      dispatch(fetchwishList(userId));
      toast.success("Wishlist emptied successfully", { toastId: "empty" });
      return response.data;
    } catch (error) {
      console.log(error);
      return rejectWithValue(error.response?.data?.error || error.message);
...
export const removeFromwishList = createAsyncThunk(
  "wishListSlice/removeFromwishList",
  ...
  extraReducers: (builder) => {
    ...
    builder
          ...
          .addCase(emptyWishlist.rejected, (state, action) => {
            state.error = action.payload;
            state.loading = false;
            toast.error("Failed to empty the wishlist", {toastId:"emptyWishlist"});
          });
  },
});
